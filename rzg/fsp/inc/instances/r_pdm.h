...
typedef enum e_pdm_short_circuit_enable
{
    ...
    PDM_SHORT_CIRCUIT_ENABLED  = (1 << R_PDM_PDSDCRCH0_SCDE_Pos), ///< Short circuit enabled
...
typedef enum e_pdm_overvoltage_lower_limit_enable
{
    ...
    PDM_OVERVOLTAGE_LOWER_LIMIT_ENABLED  = (1 << R_PDM_PDSDCRCH0_OVLDE_Pos), ///< Overvoltage lower limit enabled
...
typedef enum e_pdm_overvoltage_upper_limit_enable
{
    ...
    PDM_OVERVOLTAGE_UPPER_LIMIT_ENABLED  = (1 << R_PDM_PDSDCRCH0_OVUDE_Pos), ///< Overvoltage upper limit enabled
...
typedef enum e_pdm_buffer_overwrite_detection_enable
{
    ...
    PDM_BUFFER_OVERWRITE_DETECTION_ENABLED  = (1 << R_PDM_PDSDCRCH0_BFOWDE_Pos), ///< Buffer overwrite detection enabled
...
typedef struct st_pdm_instance_ctrl
{
    ...
    void   * p_rx_fifo;                // Read pointer for p_rx_dest, determines where to store the next sample (block of samples for DMA)
