...
#define SCI_UART_NUM_DIVISORS_ASYNC             (9U)
...
#define SCI_UART_DAT_MASK                       (0x1FFU)
...
#define SCI_UART_SEMR_ABCS0_OFFSET              (0U)
...
fsp_err_t R_SCI_UART_Open (uart_ctrl_t * const p_api_ctrl, uart_cfg_t const * const p_cfg)
{
     ...
     * or the relevant section for the MPU being used. */
    p_ctrl->p_reg->SCR = 0U;
    p_ctrl->p_reg->SSR = 0U;
...
fsp_err_t R_SCI_UART_Write (uart_ctrl_t * const p_api_ctrl, uint8_t const * const p_src, uint32_t const bytes)
{
#if (SCI_UART_CFG_TX_ENABLE)
    sci_uart_instance_ctrl_t * p_ctrl = (sci_uart_instance_ctrl_t *) p_api_ctrl;
 #if SCI_UART_CFG_PARAM_CHECKING_ENABLE || SCI_UART_CFG_DMAC_ENABLE
    fsp_err_t err = FSP_SUCCESS;
 #endif
     ...
     * method is used instead of setting TE and TIE at the same time as recommended in the user's manual to avoid
     * the one frame delay that occurs when the TE bit is set. */
    if (2U == p_ctrl->data_bytes)
    {
        p_ctrl->p_reg->TDRHL = *((uint16_t *) (p_src)) | (uint16_t) ~(SCI_UART_DAT_MASK);
    }
    else
    {
                                    ...
                                    baud_setting_t * const p_baud_setting)
{
     ...
     *  In table g_async_baud, divisor values are stored for BGDM, ABCS and N values.  Each set of divisors
                                               ...
                                               uint32_t const                         bytes)
{
    ...
    if (2U == p_ctrl->data_bytes)
    {
        ...
        FSP_ERROR_RETURN((0U == ((uint32_t) (uintptr_t) addr & SCI_UART_ALIGN_2_BYTES)), FSP_ERR_INVALID_ARGUMENT);
                                                ...
                                                uint32_t                         sci_buffer_address)
{
    ...
    *p_transfer_reg = sci_buffer_address;
    ...
    {
        /* Change transfer size to 2 bytes. */
        p_transfer->p_cfg->p_info->src_size  = TRANSFER_SIZE_2_BYTE;
        p_transfer->p_cfg->p_info->dest_size = TRANSFER_SIZE_2_BYTE;

        /* Casting for compatibility with 7 or 8 bit mode. */
        if (p_transfer == p_ctrl->p_cfg->p_transfer_tx)
        {
            *p_transfer_reg = (uint32_t) (uintptr_t) &(p_ctrl->p_reg->TDRHL);
        }
        else
        {
            *p_transfer_reg = (uint32_t) (uintptr_t) &(p_ctrl->p_reg->RDRHL);
        }
    }
    else
    {
...
static fsp_err_t r_sci_uart_transfer_open (sci_uart_instance_ctrl_t * const p_ctrl, uart_cfg_t const * const p_cfg)
{
    ...
    if (NULL != p_cfg->p_transfer_rx)
    {
        ...
        err =
            r_sci_uart_transfer_configure(p_ctrl, p_cfg->p_transfer_rx, (uint32_t *) &p_info->p_src,
                                          (uint32_t) (uintptr_t) &(p_ctrl->p_reg->RDR));
        FSP_ERROR_RETURN(FSP_SUCCESS == err, err);
    }
 #endif
 #if (SCI_UART_CFG_TX_ENABLE)
    ...
    if (NULL != p_cfg->p_transfer_tx)
    {
        ...
        err =
            r_sci_uart_transfer_configure(p_ctrl, p_cfg->p_transfer_tx, (uint32_t *) &p_info->p_dest,
                                          (uint32_t) (uintptr_t) &(p_ctrl->p_reg->TDR));
...
static void r_sci_uart_config_set (sci_uart_instance_ctrl_t * const p_ctrl, uart_cfg_t const * const p_cfg)
{
    ...
    {
        p_ctrl->p_reg->SECR = R_SCI_SECR_CSTPEN_Msk | R_SCI_SECR_CTSE_Msk;
    }
    else
#endif
    {
        p_ctrl->p_reg->SECR = ((uint8_t) (p_extend->flow_control << R_SCIG_SECR_CTSE_Pos) & R_SCIG_SECR_CTSE_Msk);
    }
...
void sci_uart_txi_isr (void)
{
    ...
    if ((NULL == p_ctrl->p_cfg->p_transfer_tx) && (0U != p_ctrl->tx_src_bytes))
    {
        ...
        if ((2U == p_ctrl->data_bytes))
        {
            ...
            p_ctrl->p_reg->TDRHL = *((uint16_t *) (p_ctrl->p_tx_src)) | (uint16_t) ~(SCI_UART_DAT_MASK);
        }
        else
        {
...
void sci_uart_eri_isr (void)
{
    ...
    if (2U == p_ctrl->data_bytes)
    {
        data = p_ctrl->p_reg->RDRHL & SCI_UART_DAT_MASK;
    }
    else
    {
        data = p_ctrl->p_reg->RDR;
